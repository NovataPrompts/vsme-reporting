      ...
      {!onlyShowStepButton && (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button 
              variant="outline" 
              className="inline-flex w-fit items-center gap-2 rounded-full bg-slate-100 hover:bg-slate-200 text-[#4e6179] border-0 focus:ring-0 focus:ring-offset-0 focus-visible:ring-0 focus-visible:outline-none"
            >
              <span>More Options</span>
              <ChevronDown className="h-4 w-4" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start" alignOffset={-2} className="w-48 bg-white">
            <DropdownMenuItem 
